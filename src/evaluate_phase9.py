...
def main():
    print("--- Phase 9 Experiment G Evaluation ---")
    
    experiments = [
        ('G1_NoWeights', 'No Weights'),
        ('G2_Cap5', 'Cap = 5'),
        ('G3_Cap10', 'Cap = 10'),
        ('G4_Cap20', 'Cap = 20 (V1 Baseline)'),
        ('G5_FocalLoss', 'Focal Loss')
    ]
    ...
    for exp_id, exp_name in experiments:
        ...
        results.append({
            'Experiment': exp_id,
            'Class Weight Strategy': exp_name,
    ...
    df_results.to_csv('d:/ALK/results/phase9/experiment_g_comparison.csv', index=False)
    ...
    plt.title('Macro F1 and Balanced Accuracy by Class-Weight Strategy')
    plt.legend()
    plt.grid(axis='y', alpha=0.3)
    plt.tight_layout()
    plt.savefig('d:/ALK/results/phase9/G_MacroF1_BalAcc_Comparison.png')
    plt.close()
    ...
    plt.grid(axis='y', alpha=0.3)
    plt.tight_layout()
    plt.savefig('d:/ALK/results/phase9/G_Minority_F1_Comparison.png')
    plt.close()
